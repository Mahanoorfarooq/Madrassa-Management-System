...
export default async function handler(
  ...
  if (req.method === "GET") {
    const doc = await MessRegistration.findById(id)
  ...
  if (req.method === "PUT") {
    ...
    const updated = await MessRegistration.findByIdAndUpdate(
      id,
      {
    ...
    return res
      .status(200)
      .json({ message: "اپ ڈیٹ ہو گیا", registration: updated });
  }

  if (req.method === "DELETE") {
    await MessRegistration.findByIdAndDelete(id);
