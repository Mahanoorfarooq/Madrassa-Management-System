...
export default async function handler(req: NextApiRequest, resp: NextApiResponse) {
    // Add auth checking here for super admin

    try {
        await connectDB();

        const totalJamias = await Jamia.countDocuments({ isDeleted: { $ne: true } });
        const activeJamias = await Jamia.countDocuments({ isActive: true, isDeleted: { $ne: true } });
