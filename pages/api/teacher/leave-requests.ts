...
export default async function handler(
  ...
  // Find students taught by this teacher (by class/section assignments)
  ...
  if (req.method === "GET") {
    ...
    const list = await StudentLeaveRequest.find(filter)
      .sort({ createdAt: -1 })
      .limit(200)
      .lean();
  ...
  const { requestId, status, reviewNote } = req.body as {
    requestId?: string;
    status?: LeaveStatus;
    reviewNote?: string;
  };
  ...
  const doc: any = await StudentLeaveRequest.findById(requestId).populate({
    path: "student",
    select: "_id",
  });
  ...
  if (typeof reviewNote === "string") {
    doc.reviewNote = reviewNote.trim();
  }
  doc.reviewedBy = me.id;
  await doc.save();
