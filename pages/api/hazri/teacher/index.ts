import type { NextApiRequest, NextApiResponse } from "next";
import { connectDB } from "@/lib/db";
import { requireAuth } from "@/lib/auth";
import { TeacherAttendance } from "@/schemas/TeacherAttendance";
import { logActivity } from "@/lib/activityLogger";

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse
) {
  const user = requireAuth(req, res, ["admin", "teacher", "staff"]);
  if (!user) return;
  await connectDB();

  if (req.method === "GET") {
    const { teacherId, from, to } = req.query as any;
    const filter: any = {};
    if (teacherId) filter.teacherId = teacherId;
    if (from || to) {
      filter.date = {} as any;
      if (from) filter.date.$gte = new Date(from);
      if (to) {
        const end = new Date(to);
        end.setHours(23, 59, 59, 999);
        filter.date.$lte = end;
      }
    }
    const list = await TeacherAttendance.find(filter)
      .populate({ path: "teacherId", select: "name" })
      .sort({ date: -1 })
      .limit(500);
    return res.status(200).json({ attendance: list });
  }

  if (req.method === "POST") {
    if (user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "صرف ایڈمن حاضری محفوظ کر سکتا ہے" });
    }
    const body = req.body as any;
    const date = new Date(body.date);
    date.setHours(0, 0, 0, 0);
    const rec = await TeacherAttendance.findOneAndUpdate(
      { teacherId: body.teacherId, date },
      { status: body.status, markedBy: user.id },
      { upsert: true, new: true, setDefaultsOnInsert: true }
    );
    await logActivity({
      actorUserId: user.id,
      action: "teacher_attendance_marked",
      entityType: "teacher_attendance",
      entityId: rec?._id,
      after: { teacherId: body.teacherId, date, status: body.status },
      meta: { markedBy: user.id, role: user.role },
    });
    return res
      .status(201)
      .json({ message: "حاضری محفوظ ہو گئی", attendance: rec });
  }

  return res.status(405).json({ message: "غیر مجاز میتھڈ" });
}
