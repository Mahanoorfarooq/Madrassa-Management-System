...
export default function HifzDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // Ensure the HIFZ department exists and get stats
  ...
  const quickLinks = [
    { href: "/talba/students?dept=HIFZ", label: "نیا طالب علم شامل کریں" },
    { href: "/talba/teachers?dept=HIFZ", label: "نیا استاد شامل کریں" },
    { href: "/talba/classes?dept=HIFZ", label: "نئی کلاس بنائیں" },
    { href: "/talba/attendance?dept=HIFZ", label: "آج کی حاضری لگائیں" },
  ];

  return (
    <TalbaLayout title="شعبہ حفظ القرآن ڈیش بورڈ">
      {error && (
