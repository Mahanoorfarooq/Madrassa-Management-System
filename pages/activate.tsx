...
export default function ActivatePage() {
  ...
  useEffect(() => {
    ...
    try {
      const pending = localStorage.getItem("pending_activation_license_key");
      if (pending && pending.trim()) {
        setLicenseKey(pending.trim().toUpperCase());
        localStorage.removeItem("pending_activation_license_key");
      }
    } catch {
      // ignore
    }
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "لائسنس کی غلط ہے یا ایکسپائر ہو چکی ہے",
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="p-6 space-y-6">
            ...
            {success && (
              <div className="bg-green-50 text-green-600 text-[10px] px-4 py-3 rounded-lg text-center border border-green-100 font-urdu">
                سافٹ ویئر کامیابی سے ایکٹیویٹ ہو گیا ہے! ری ڈائریکٹ کیا جا رہا
                ہے...
              </div>
            )}
