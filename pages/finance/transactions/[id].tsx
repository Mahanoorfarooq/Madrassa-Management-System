...
export default function EditFinanceTransactionPage() {
  ...
  useEffect(() => {
    const loadTxn = async () => {
      ...
      setLoading(true);
      try {
        const res = await api.get(`/api/finance/transactions/${id}`);
        const t = res.data?.transaction;
        if (t) {
          setType(t.type || "student_fee");
          setAmount(Number(t.amount || 0));
          setDate(
            t.date ? new Date(t.date).toISOString().substring(0, 10) : ""
          );
          setDescription(t.description || "");
          setDepartment(t.department?._id || t.department || "");
          setStudent(t.student?._id || t.student || "");
          setTeacher(t.teacher?._id || t.teacher || "");
        }
      } catch (e: any) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.put(`/api/finance/transactions/${id}`, {
        type,
      ...
      <div className="space-y-4" dir="rtl">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl shadow-md p-5 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">معاملہ ترمیم کریں</h1>
              <p className="text-blue-100 text-xs">
                منتخب معاملے کی تفصیلات تبدیل کریں
              </p>
            </div>
          </div>
        </div>
