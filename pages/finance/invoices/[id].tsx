...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    const boot = async () => {
      if (!id) return;
      const [dRes, sRes, inv] = await Promise.all([
      ...
      setForm(inv.data?.invoice || { items: [], status: "unpaid" });
      setLoading(false);
    };
    boot();
  }, [id]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.put(`/api/finance/invoices/${id}`, {
  ...
  if (loading)
    return <FinanceLayout title="انوائس">لوڈ ہو رہا ہے...</FinanceLayout>;

  return (
    <FinanceLayout title="انوائس میں ترمیم">
      <form
        onSubmit={onSubmit}
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              type="date"
              value={form.dueDate ? String(form.dueDate).substring(0, 10) : ""}
