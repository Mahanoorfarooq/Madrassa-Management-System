...
export default function EditKitchenPage() {
  ...
  useEffect(() => {
    const load = async () => {
      if (!id) return;
      const r = await api.get(`/api/mess/kitchens/${id}`);
      setForm(r.data?.kitchen || {});
      setLoading(false);
    };
    load();
  }, [id]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.put(`/api/mess/kitchens/${id}`, {
        name: form.name,
  ...
  if (loading) return <MessLayout title="کچن">لوڈ ہو رہا ہے...</MessLayout>;

  return (
    <MessLayout title="کچن میں ترمیم">
      <form
        onSubmit={onSubmit}
